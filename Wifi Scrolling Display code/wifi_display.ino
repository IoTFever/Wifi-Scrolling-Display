//Wifi messages scrolling display using nodemcu
//made by Dhananjay 
 ...
 * DIN (data in) on Matrix ---> 13 or MOSI on ESP8266          D7
 * Clock(CLK) on Matrix --> 14 or SCK on ESP8266               D5
 * CS pin on Matrix define below  --->( pick 15 on esp8266)    D8
 */
...
void loop() 
{
  ...
  else
  {
    ...
    s = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n";
    s+="<!doctype html> <style> \#header{ min-height: 20px; background-color: \#666699; } \#menu{ min-height: 20px; margin-top: 1%; background-color: \#999999; } \#body{ min-height: 200px; margin-top: 1%; } \#footer{ min-height: 20px; margin-top: 1%; background-color: \#666699; } \#header, \#menu, \#body, \#footer{ margin-left: 10%; margin-right: 10%; box-shadow: 3px 5px 7px \#666666; border: 1px solid black; } @viewport{ zoom: 1.0; width: extend-to-zoom; } @-ms-viewport{ width: extend-to-zoom; zoom: 1.0; } </style> <html lang='en'> <head> <meta name='viewport' content='width=device-width, initial-scale=1'><div><br> <form action='esp8266'> <p>Made by Dhananjay</p> <br>Enter Text to display on LED Matrix:<br><input type='text' maxlength='70' name='max' value=''><br><br> <input type='submit' value='Submit'></form> </body>";
    s += "</html>\r\n\r\n";
    client.print(s);
   return;
  }
